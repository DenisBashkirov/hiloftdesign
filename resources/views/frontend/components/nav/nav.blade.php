<a class="header-nav-item" href="{{ route('portfolio') }}">Портфолио</a>
<span>
    <span>Услуги и цены</span>
    <div class="header-nav-dropdown hidden absolute flex flex-col left-0 sy-3 py-6 px-3 white-space-nowrap b-radius-2 bg-grey-80">
        <a class="header-nav-dropdown-item" href="{{ route('services.dizayn_kvartir') }}">Дизайн квартир</a>
        <a class="header-nav-dropdown-item" href="#">Дизайн домов и коттеджей</a>
    </div>
</span>
<a class="header-nav-item" href="#">Блог</a>
<a class="header-nav-item" href="#">Чек-листы</a>
<a class="header-nav-item" href="#">Контакты</a>
